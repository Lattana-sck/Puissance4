class P4{
    constructor(selector, COL = 7, LGN = 6, colorP1 = 'red', colorP2 = 'yellow', nameP1 = 'joueur1', nameP2 = 'joueur2'){
    ...
    recherche(){
        ...
        document.getElementById("turn").innerHTML = "C'est au tour de " + that.nameP1 + " de jouer !";
        ...
        $jeu.on('mouseenter', '.col.empty', function(){
            ...
            if($last != null){
                $last.addClass('p' + that.colorPlayer).css('background-color', that.colorPlayer);
            }
        });
        ...
        $jeu.on('click', '.col.empty', function(){
            ...
            $last.removeClass('empty p' + that.colorPlayer).data('player', that.colorPlayer).addClass(that.colorPlayer).css("background-color", that.colorPlayer).addClass('fall');
            ...
            if (that.colorPlayer === colorP1) {
                document.getElementById("turn").innerHTML = "C'est au tour de " + that.nameP2 + " de jouer !";
            }
            else{
                document.getElementById("turn").innerHTML = "C'est au tour de " + that.nameP1 + " de jouer !";
            }
...
// verifier si un des deux joueurs a gagné.
    checkWin(lgn, col){
        ...
        function checkDirection(direction){
            ...
            while(i >= 0 && i < that.LGN && j >= 0 && j < that.COL && $next.data('player') === that.colorPlayer ){
                total++;
                i += direction.i;
                j += direction.j;
                $next = $getCell(i, j);
